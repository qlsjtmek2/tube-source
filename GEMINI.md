...
app/
  ...
  page.tsx                  # Main UI (검색, 결과, 트렌드, 분석 기록 탭)
...
components/
  ...
  analysis-dialog.tsx       # AI 분석 결과 다이얼로그 (저장 기능 포함)
...
store/
  search-context.tsx        # Global state (검색어, 필터, 영상 결과, 분석 기록)
...
3. **AI Analysis Flow**
   ...
   - User can click "저장" to save analysis result locally

4. **Analyzed Videos History Flow**
   - User saves analysis → POST to `/api/analyzed-videos`
   - `lib/storage.ts` appends to `data/analyzed-videos.json`
   - User visits "분석 기록" tab → GET `/api/analyzed-videos`
   - Displays list of previously analyzed videos
   - DELETE removes analysis record

5. **Subtitle Viewing Flow**
...
- **Global Search State**: `SearchContext` (store/search-context.tsx)
  - Holds: query, filters, videos[], hasSearched, analyzedVideos[]
...
- `lib/storage.ts`는 `data/channels.json` 및 `data/analyzed-videos.json`을 직접 읽고 쓴다
